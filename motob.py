...
class Motob:
    ...
    def decodeMR(self, motor_recommendation): #in form ("command", angle)
        ...
        if command == "goForward":
            self.motors.forward(self.angle_to_speed(angle))
        elif command == "turn":
            if angle > 0:
                self.motors.right(self.angle_to_speed(angle))
            else:
                self.motors.left(self.angle_to_speed(angle))
        elif command == "goBackward":
            self.motors.backward(self.angle_to_speed(angle))
